...
public class PaperNetworkModel extends DefaultNetworkModel {
	...
	public void processEvent(SimEvent ev) {
		...
		case SEND_RESULT_FROM_ORCH_TO_DEV:
			...
			break;
		case OPTIMIZATION_REQUEST_SENT:
			sendOptimizationRequest((OrchOptRequest)ev.getData());
			break;
		case OPTIMIZATION_REQUEST_RESPONSE_SENT:
			sendOptimizationRequestResponse((OrchOptRequest)ev.getData());
			break;
		case OPTIMIZATION_REQUEST_CONFIRMED_SENT:
			sendOptimizationRequestConfirmed((OrchOptRequest)ev.getData());
			break;
		case OPTIMIZATION_REQUEST_NOT_CONFIRMED_SENT:
			sendOptimizationRequestNotConfirmed((OrchOptRequest)ev.getData());
			break;
	...
	private void sendOptimizationRequest(OrchOptRequest Request){
		FileTransferProgress FTP = new FileTransferProgress(null, PaperSettings.ORCH_TO_ORCH_MSG_SIZE*8, FileTransferProgress.Type.OPTIMIZATION_REQUEST_SENT);
		FTP.setRequest(Request);
		transferProgressList.add(FTP);
	}
	
	private void sendOptimizationRequestResponse(OrchOptRequest Request){
		FileTransferProgress FTP = new FileTransferProgress(null, PaperSettings.ORCH_TO_ORCH_MSG_SIZE*8, FileTransferProgress.Type.OPTIMIZATION_REQUEST_RESPONSE_SENT);
		FTP.setRequest(Request);
		transferProgressList.add(FTP);	
	}
	
	private void sendOptimizationRequestNotConfirmed(OrchOptRequest Request){
		FileTransferProgress FTP = new FileTransferProgress(null, PaperSettings.ORCH_TO_ORCH_MSG_SIZE*8, FileTransferProgress.Type.OPTIMIZATION_REQUEST_NOT_CONFIRMED_SENT);
		FTP.setRequest(Request);
		transferProgressList.add(FTP);	
	}
	
	private void sendOptimizationRequestConfirmed(OrchOptRequest Request){
		FileTransferProgress FTP = new FileTransferProgress(null, PaperSettings.ORCH_TO_ORCH_MSG_SIZE*8, FileTransferProgress.Type.OPTIMIZATION_REQUEST_CONFIRMED_SENT);
		FTP.setRequest(Request);
		transferProgressList.add(FTP);	
	}
	...
	protected void transferFinished(FileTransferProgress transfer) {
		...
		else if (transfer.getTransferType() == FileTransferProgress.Type.OPTIMIZATION_REQUEST_SENT) {
			double latency = CalculateLatency(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest());
			schedule(transfer.getRequest().getDestRequest(), latency, PaperEdgeDevice.OPTIMIZATION_REQUEST_RECEIVED,transfer.getRequest());
			updateEnergyConsumption(transfer, "Request_sent");
		}
		
		else if (transfer.getTransferType() == FileTransferProgress.Type.OPTIMIZATION_REQUEST_RESPONSE_SENT) {
			double latency = CalculateLatency(transfer.getRequest().getDestRequest(), transfer.getRequest().getOriginRequest());
			schedule(transfer.getRequest().getOriginRequest(), latency, PaperEdgeDevice.OPTIMIZATION_REQUEST_RESPONSE,transfer.getRequest());
			updateEnergyConsumption(transfer, "Request_response_sent");
		}
		else if (transfer.getTransferType() == FileTransferProgress.Type.OPTIMIZATION_REQUEST_CONFIRMED_SENT) {
			double latency = CalculateLatency(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest());
			schedule(transfer.getRequest().getDestRequest(), latency, PaperEdgeDevice.OPTIMIZATION_REQUEST_CONFIRMED,transfer.getRequest());
			updateEnergyConsumption(transfer, "Request_confirmed_sent");
		}
		else if (transfer.getTransferType() == FileTransferProgress.Type.OPTIMIZATION_REQUEST_NOT_CONFIRMED_SENT) {
			double latency = CalculateLatency(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest());
			schedule(transfer.getRequest().getDestRequest(), latency, PaperEdgeDevice.OPTIMIZATION_REQUEST_NOT_CONFIRMED,transfer.getRequest());
			updateEnergyConsumption(transfer, "Request_not_confirmed_sent");
		}
		// If it is an task (or offloading request) that is sent to the destination
			...
			if (SimulationParameters.ENABLE_REGISTRY && "CLOUD".equals(SimulationParameters.registry_mode)
				...
				schedule(this, latency, DefaultNetworkModel.DOWNLOAD_CONTAINER, transfer.getTask());
		...
		// If the transfer of execution results to the orchestrator has finished
			...
			if (transfer.getTask().getOrchestrator().getType().equals(TYPES.CLOUD)
					|| ((DataCenter) transfer.getTask().getVm().getHost().getDatacenter()).getType().equals(TYPES.CLOUD))
				schedule(this, SimulationParameters.WAN_PROPAGATION_DELAY + latency, DefaultNetworkModel.SEND_RESULT_FROM_ORCH_TO_DEV,
						transfer.getTask());
			else
				schedule(this,latency, DefaultNetworkModel.SEND_RESULT_FROM_ORCH_TO_DEV, transfer.getTask());
	...
	protected void updateEnergyConsumption(FileTransferProgress transfer, String type) {
		...
		else if ("Request_sent".equals(type)) {
			calculateEnergyConsumption(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest(), transfer);
		}
		else if ("Request_response_sent".equals(type)) {
			calculateEnergyConsumption(transfer.getRequest().getDestRequest(), transfer.getRequest().getOriginRequest(), transfer);
		}
		else if ("Request_confirmed_sent".equals(type)) {
			calculateEnergyConsumption(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest(), transfer);
		}
		else if ("Request_not_confirmed_sent".equals(type)) {
			calculateEnergyConsumption(transfer.getRequest().getOriginRequest(), transfer.getRequest().getDestRequest(), transfer);
		}
	...
	protected boolean sameLanIsUsed(FileTransferProgress FTP1, FileTransferProgress FTP2) {
		Task task1 = FTP1.getTask();
		Task task2 = FTP2.getTask();
		if(task1==null && task2==null)
			return 	// Compare origin and dest
					((FTP1.getRequest().getOriginRequest() == FTP2.getRequest().getOriginRequest())
					|| (FTP1.getRequest().getOriginRequest() == FTP2.getRequest().getDestRequest())
					|| (FTP1.getRequest().getDestRequest() == FTP2.getRequest().getDestRequest()));
	
		if(task1==null)		
			return 	// Compare origin device
					((FTP1.getRequest().getOriginRequest() == task2.getOrchestrator())
					|| (FTP1.getRequest().getOriginRequest() == task2.getVm().getHost().getDatacenter())
					|| (FTP1.getRequest().getOriginRequest() == task2.getEdgeDevice())
					|| (FTP1.getRequest().getOriginRequest() == task2.getRegistry())

					// Compare offloading destination
					|| (FTP1.getRequest().getDestRequest() == task2.getOrchestrator())
					|| (FTP1.getRequest().getDestRequest() == task2.getVm().getHost().getDatacenter())
					|| (FTP1.getRequest().getDestRequest() == task2.getEdgeDevice())
					|| (FTP1.getRequest().getDestRequest() == task2.getRegistry()));	
		
		if(task2==null)		
			return 	// Compare origin device
					((FTP2.getRequest().getOriginRequest() == task1.getOrchestrator())
					|| (FTP2.getRequest().getOriginRequest() == task1.getVm().getHost().getDatacenter())
					|| (FTP2.getRequest().getOriginRequest() == task1.getEdgeDevice())
					|| (FTP2.getRequest().getOriginRequest() == task1.getRegistry())

					// Compare offloading destination
					|| (FTP2.getRequest().getDestRequest() == task1.getOrchestrator())
					|| (FTP2.getRequest().getDestRequest() == task1.getVm().getHost().getDatacenter())
					|| (FTP2.getRequest().getDestRequest() == task1.getEdgeDevice())
					|| (FTP2.getRequest().getDestRequest() == task1.getRegistry()));
		
		return  // Compare orchestrator
				((task1.getOrchestrator() == task2.getOrchestrator())
			...
			if (transferProgressList.get(i).getRemainingFileSize() > 0) {
				...
				for (int j = 0; j < transferProgressList.size(); j++) {
					...
					if (transferProgressList.get(j).getRemainingFileSize() > 0 && j != i && sameLanIsUsed(
							transferProgressList.get(i), transferProgressList.get(j))) {
