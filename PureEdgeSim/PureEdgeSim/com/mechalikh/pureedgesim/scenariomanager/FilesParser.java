...
public class FilesParser {
	...
	private boolean checkSimulationProperties(String simProp) {
		...
		try {
			...
			SimulationParameters.EDGE_DATACENTERS_RANGE_MIN = Integer
					.parseInt(prop.getProperty("edge_datacenters_coverage_min").trim()); // meters
			SimulationParameters.EDGE_DATACENTERS_RANGE_MAX = Integer
			...
			try {
				SimulationParameters.SEED = Long.parseLong(prop.getProperty("seed").trim());
			} catch (Exception e) {
				Random ran = new Random();
				SimulationParameters.SEED = ran.nextLong(); 			
			}
			
			SimulationParameters.EDGE_DEVICE_COUNTER_STEP = Integer
					.parseInt(prop.getProperty("edge_device_counter_size").trim());
			
			SimulationParameters.DEVICES_DISTRIBUTION = prop.getProperty("distribution").trim();
	...
	private boolean checkAppFile(String appFile) {
		...
		try {
			...
			for (int i = 0; i < appList.getLength(); i++) {
				...
				isElementPresent(appElement, "required_bandwidth");
				isElementPresent(appElement, "bandwidth_increment");
				isElementPresent(appElement, "MCU_bandwidth_linear");
				isElementPresent(appElement, "MCU_bandwidth_exponential");
				isElementPresent(appElement, "MCU_bandwidth_linear_standard_deviation");
				isElementPresent(appElement, "MCU_bandwidth_exponential_standard_deviation");				
				isElementPresent(appElement, "required_ram");
				isElementPresent(appElement, "ram_increment");
				isElementPresent(appElement, "core_increment");
				isElementPresent(appElement, "MCU_CPU_linear");
				isElementPresent(appElement, "MCU_CPU_exponential");
				isElementPresent(appElement, "MCU_CPU_linear_standard_deviation");
				isElementPresent(appElement, "MCU_CPU_exponential_standard_deviation");
				isElementPresent(appElement, "MCU_RAM_linear");
				isElementPresent(appElement, "MCU_RAM_exponential");
				isElementPresent(appElement, "MCU_RAM_linear_standard_deviation");
				isElementPresent(appElement, "MCU_RAM_exponential_standard_deviation");		
				isElementPresent(appElement, "max_link_latency");
				...
				// increment of cpu core needed, per user
				double core_increment = Double
						.parseDouble(appElement.getElementsByTagName("core_increment").item(0).getTextContent());
				
				// increment, as a floating point value 0 to max_float, of cpu core needed in case of linear increment of MCU
				double MCU_CPU_linear = Double
						.parseDouble(appElement.getElementsByTagName("MCU_CPU_linear").item(0).getTextContent());	
				
				double MCU_CPU_exponential = Double
						.parseDouble(appElement.getElementsByTagName("MCU_CPU_exponential").item(0).getTextContent());
				
				double MCU_CPU_linear_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_CPU_linear_standard_deviation").item(0).getTextContent());
				
				double MCU_CPU_exponential_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_CPU_exponential_standard_deviation").item(0).getTextContent());
				
				// required initial RAM
				int required_ram = Integer
						.parseInt(appElement.getElementsByTagName("required_ram").item(0).getTextContent());
				
				// increment of ram needed, in megabytes, per user
				int ram_increment = Integer
						.parseInt(appElement.getElementsByTagName("ram_increment").item(0).getTextContent());				
			
				double MCU_RAM_linear = Double
						.parseDouble(appElement.getElementsByTagName("MCU_RAM_linear").item(0).getTextContent());	
				
				double MCU_RAM_exponential = Double
						.parseDouble(appElement.getElementsByTagName("MCU_RAM_exponential").item(0).getTextContent());
				
				double MCU_RAM_linear_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_RAM_linear_standard_deviation").item(0).getTextContent());
				
				double MCU_RAM_exponential_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_RAM_exponential_standard_deviation").item(0).getTextContent());
				
				// required initial bandwidth 
				int required_bandwidth = Integer
						.parseInt(appElement.getElementsByTagName("required_bandwidth").item(0).getTextContent());	
				
				// increment of bandwidth, in megabit/s, per user 
				int bandwidth_increment = Integer
						.parseInt(appElement.getElementsByTagName("bandwidth_increment").item(0).getTextContent());
				
				double MCU_bandwidth_linear = Double
						.parseDouble(appElement.getElementsByTagName("MCU_bandwidth_linear").item(0).getTextContent());
				
				double MCU_bandwidth_exponential = Double
						.parseDouble(appElement.getElementsByTagName("MCU_bandwidth_exponential").item(0).getTextContent());
				
				double MCU_bandwidth_linear_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_bandwidth_linear_standard_deviation").item(0).getTextContent());
				
				double MCU_bandwidth_exponential_standard_deviation = Double
						.parseDouble(appElement.getElementsByTagName("MCU_bandwidth_exponential_standard_deviation").item(0).getTextContent());
				
				// max latency from the container of the app to users 
				double max_link_latency = Double
						.parseDouble(appElement.getElementsByTagName("max_link_latency").item(0).getTextContent());
				
				
				// save apps parameters
				SimulationParameters.APPLICATIONS_LIST.add(new Application(rate, usage_percentage, max_delay,
						container_size, request_size, results_size, task_length, required_cores, core_increment, MCU_CPU_linear, MCU_CPU_exponential,  
						MCU_CPU_linear_standard_deviation, MCU_CPU_exponential_standard_deviation, 	required_ram,  ram_increment, MCU_RAM_linear,
						MCU_RAM_exponential, MCU_RAM_linear_standard_deviation, MCU_RAM_exponential_standard_deviation, required_bandwidth, 
						bandwidth_increment, MCU_bandwidth_linear, MCU_bandwidth_exponential, MCU_bandwidth_linear_standard_deviation, 
						MCU_bandwidth_exponential_standard_deviation, max_link_latency));
