...
public class SimulationManager extends SimulationManagerAbstract {
	...
	public SimulationManager(SimLog simLog, CloudSim simulation, int simulationId, int iteration, Scenario scenario, long seed) {
	...
	public void processEvent(SimEvent ev) {
		...
		case PRINT_LOG:
			...
			ListIterator<DataCenter> ListOrchIter =  getDataCentersManager().getOrchestratorsList().listIterator();
			while(ListOrchIter.hasNext())
			{
				DataCenter Orchestrator = ListOrchIter.next();		
				Vm VM = Orchestrator.getResources().getVmList().get(0);
				System.out.println("---------------------------------------------------------------------------------------------------------");
				System.out.println("-----------------------SITUAZIONE ALLA FINE DELLA SIMULAZIONE AL TEMPO "+ getSimulation().clock() +" -------------------------------");
				...
				System.out.printf("Edge Datacenter " + Orchestrator.getId() + " | Vm %d CPU Usage: %d vCPUs on %d vCPUs | Running Tasks: #%d | RAM usage: %d MB on %d MB \n",
						 VM.getId(), VM.getFreePesNumber(), VM.getNumberOfPes(),
	...
	private boolean sameLocation(DataCenter Dev1, DataCenter Dev2) {
		...
		if (Dev1.getType() != Dev2.getType())
		{
			// One of them is an edge data center and the other is an edge device
